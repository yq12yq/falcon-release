...
public class OozieProcessWorkflowBuilderTest extends AbstractTestBase {
    ...
    private COORDINATORAPP getCoordinator(Path path) throws Exception {
        ...
        JAXBElement<COORDINATORAPP> jaxbBundle = unmarshaller.unmarshal(
                new StreamSource(new ByteArrayInputStream(bundleStr.trim().getBytes())),
                COORDINATORAPP.class);
    ...
    public void testProcessWithInputsNoOutputs() throws Exception {
        ...
        Assert.assertEquals(props.get("logDir"), getLogPath(processEntity));
        Assert.assertEquals(props.get(EntityInstanceMessage.ARG.feedNames.getPropName()), "NONE");
    }
    ...
    public void testProcessNoInputsWithOutputs() throws Exception {
        ...
        Assert.assertEquals(props.get(EntityInstanceMessage.ARG.feedNames.getPropName()), "impressions");
    }
}
