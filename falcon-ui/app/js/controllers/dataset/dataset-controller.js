          ...
          $scope.model.properties.property.forEach(function (item) {
            ...
            if (item._name === 'drSourceDir') {
              ...
              $scope.UIModel.source.path.split(',').forEach(function(sourcePath, index, sourcePaths) {
                if (!sourcePath.startsWith('hdfs://240.0.0.10:8020')) {
                  item._value = item._value.concat('hdfs://240.0.0.10:8020');
                }
            ...
            if (item._name === 'drTargetDir') {
              ...
              $scope.UIModel.target.path.split(',').forEach(function(targetPath, index, targetPaths) {
                item._value = item._value.concat(targetPath.replace('hdfs://240.0.0.10:8020', ''));
