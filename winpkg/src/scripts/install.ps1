...
###
### Install script that can be used to install falcon as a Single-Node cluster.
...
###   falcon services as Windows services.
...
{
    $FinalName = "falcon-0.10-SNAPSHOT"
    ...
    if ( $ENV:IS_DISTRIBUTED -eq "yes" ) {
      $falcontype = "distributed"
      Write-Log "Type is $falcontype"
    }
...
{ 
    ...
    $utilsModule = Import-Module -Name "$scriptDir\..\resources\Winpkg.Utils.psm1" -ArgumentList ("falcon") -PassThru
